...
The entire website supports translation into 22 languages with automatic language detection. Admin authentication uses DB-backed user accounts (`admin_users` table) with bcrypt-equivalent scrypt password hashing and three roles: **super_admin** (full access + manages users + views audit log), **editor** (create/edit/delete content), **viewer** (read-only). On first run, a super_admin is bootstrapped from `ADMIN_USERNAME`/`ADMIN_PASSWORD` env vars. Super admins manage users at `/admin/users` (add/disable/delete/reset password/change role) and review the full audit log at `/admin/audit-log` — every successful mutating admin request is recorded with user, role, method, path, status, redacted body preview, IP, user agent, and timestamp (`audit_log` table). Sessions store `adminUserId`/`adminUsername`/`adminRole` and are server-side persisted. SEO is managed with `react-helmet-async` for dynamic meta tags, unique SEO for all public pages, and `noindex` for utility pages. A peer-to-peer fundraising system allows supporters to create personal fundraising pages linked to campaigns. Membership registration is styled as a "membership card" with visual cues and comprehensive data capture. A return request system allows customers to initiate returns, which admins can manage. Store policies pages detail quality guarantees, shipping, and return procedures. Admin quality control features allow for placing test orders and marking them as quality-checked.
