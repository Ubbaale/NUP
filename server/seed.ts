  ...
  try {
    ...
    const conferencesData = [
      ...
      },
      {
        ...
        description: "Join us at our annual convention in Boston to meet fellow Ugandans from all across the globe sharing in the same NUP initiatives. In Boston, we aim to reignite the passion and responsibility of the diaspora as catalysts for transformative change. After 38 years of authoritarian rule, we must unite and equip ourselves with the strategies, resources, and determination needed to support Uganda's transition to democracy. We encourage every Ugandan in the diaspora and friends of Uganda to attend and actively participate in this pivotal gathering.",
        theme: "Reigniting the Passion for Democratic Change",
        registrationUrl: "https://buy.stripe.com/fZe4k1f8Vg8l2B214d",
        isUpcoming: false,
        speakers: JSON.stringify(["President Robert Ssentamu Kyagulanyi (Bobi Wine)", "S.G. David Lewis Rubongeya", "Hon. Joel Ssenyonyi - Leader of Opposition", "First Lady Barbie Itungo Kyagulanyi"]),
      },
      {
        ...
        description: "Delegates can look forward to immersing themselves in the vibrant tapestry of Chicago's landmarks, culture, and history. From the towering skyscrapers along the Magnificent Mile to the serene beauty of Lake Michigan's shores, Chicago promised an unforgettable experience. The convention offered a unique opportunity to connect with like-minded individuals, both Ugandans in the Diaspora and friends of Uganda, engaging in discussions on pressing issues, networking with activists, and learning from global leaders in the realms of democracy and governance. Over 500 NUP supporters gathered to forge a united front for Uganda's future.",
        theme: "Celebrating Democracy and Unity",
        isUpcoming: false,
        speakers: JSON.stringify(["President Robert Ssentamu Kyagulanyi (Bobi Wine)", "Congressman Danny K Davis", "Professor David Ssejinja", "Professor James Powell", "Professor Tim Szczepanski", "Katie Lowe - CFO, American Leaders Class", "Hon. Joel Ssenyonyi - Leader of Opposition", "Professor Milton Allimadi"]),
      },
      {
    ...
    for (const conferenceData of conferencesData) {
      const existing = await storage.getConferenceBySlug(conferenceData.slug);
      if (existing) {
        await storage.updateConference(existing.id, conferenceData);
      } else {
    ...
    const tiersData = [
      {
        name: "NUP Diaspora Gold",
        slug: "nup-diaspora-gold",
        price: "1000.00",
        interval: "yearly",
        description: "Premium member renewal — the highest level of commitment to the NUP Diaspora movement.",
        benefits: JSON.stringify(["Priority convention registration & VIP access", "Direct engagement with NUP leadership", "Exclusive Gold member badge & certificate", "Recognition at all NUP Diaspora events", "Advisory council invitation", "Free convention passes for family", "Monthly leadership briefings", "Premium engraved plaque shipped to you", "Name on NUP Diaspora Honor Roll"]),
        badgeColor: "#FFD700",
        awardType: "plaque",
        awardDescription: "A distinguished wooden plaque with gold engraving of your name — the highest honor for NUP Diaspora Gold members worldwide.",
        isPopular: true,
        isActive: true,
        displayOrder: 1,
      },
      {
        name: "NUP Diaspora Silver",
        slug: "nup-diaspora-silver",
        price: "500.00",
        interval: "yearly",
        description: "Member renewal — show your continued support for the NUP Diaspora movement.",
        benefits: JSON.stringify(["Convention registration access", "Exclusive Silver member badge & certificate", "Recognition at NUP Diaspora events", "Quarterly leadership updates", "Access to members-only content", "Engraved crystal award shipped to you", "Digital membership card", "NUP Diaspora newsletter"]),
        badgeColor: "#C0C0C0",
        awardType: "crystal",
